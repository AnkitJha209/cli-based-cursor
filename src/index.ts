...
const availableTools : {
    ...
    writeCode: (file: string, code: string) => string | undefined;
} = {
    ...
    writeCode: (file: string, code: string) => {
        try {
            const pathOfFile = path.resolve(file);
            const response = fs.writeFileSync(pathOfFile, code);
            return `Code written ->  ${response}`;
        }catch(err) {
            console.log(err);
        }
    }
...
const systemPrompt = `
    ...
    Available tools
    - "runCommand": Takes the cli command as string and executes the command and returns the output after executing it.
    ...
    while (true) {
        try {
            ...
            if (!response.choices[0].message.content) {
                console.log("Error: No response content");
                return;
            }
            ...
            let parsedMessage;
            try {
                parsedMessage = JSON.parse(responseContent);
            } catch (error) {
                console.log("Error parsing JSON response:", error);
                return;
            }

            if (parsedMessage.step === 'plan') {
                console.log(`🧠: ${parsedMessage.content}`);
                continue;
            }

            if (parsedMessage.step === 'action') {
                if (parsedMessage.function === 'runCommand') {
                    const toolInput = parsedMessage.input;
                    console.log(`🛠️: Calling Tool: ${parsedMessage.function} with input:`, toolInput);
                    const result = await availableTools.runCommand(toolInput);
                    console.log(`📋: Command output: ${result}`);
                    messages.push({ role: 'user', content: JSON.stringify({ step: 'observe', output: result }) });
                    continue;
                }

                if (parsedMessage.function === 'writeCode') {
                    try {
                        let toolInput;
                        if (typeof parsedMessage.input === 'string') {
                            toolInput = JSON.parse(parsedMessage.input);
                        } else {
                            toolInput = parsedMessage.input;
                        }
                        
                        const file = toolInput.file;
                        const code = toolInput.code;
                        
                        console.log(`🛠️: Calling Tool: ${parsedMessage.function} with file:`, file);
                        const result = availableTools.writeCode(file, code);
                        console.log(`📋: Write result: ${result}`);
                        messages.push({ role: 'user', content: JSON.stringify({ step: 'observe', output: result }) });
                        continue;
                    } catch (error) {
                        console.log("Error processing writeCode:", error);
                        return;
                    }
                }
            }

            if (parsedMessage.step === 'observe') {
                console.log(`👀: ${parsedMessage.content || parsedMessage.output}`);
                continue;
            }

            if (parsedMessage.step === 'output') {
                console.log(`🤖: ${parsedMessage.content}`);
                break;
            }
...
async function main() {
    console.log("AI Assistant started. Type your commands:");
    ...
    const askQuestion = () => {
        rl.question("> ", async (command) => {
            ...
            askQuestion(); // Ask for next command
        });
    };
